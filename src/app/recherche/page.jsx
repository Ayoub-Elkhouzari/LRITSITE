...
import { Search } from "lucide-react";import axes from "@/data/axes.json";
  ...
  return (
    <Card className="w-full overflow-y-auto custom-scrollbar">
      ...
      <CardContent className="grid-cols-8 justify-center items-center p-4">
        ...
        {/* <Card className="flex justify-center items-center w-full overflow-y-auto custom-scrollbar">
          <div className="w-52 h-24 p-4 text-4xl bg-transparent text-center font-bold mr-2 text-secondary-foreground" >1</div>
          <div className="w-full h-24 p-4 text-4xl bg-transparent text-center font-bold text-secondary-foreground">Intelligence artificielle et ses applications</div>
        </Card> */}
       {axes.map((axe) => (
          <Card
            key={axe.id}
